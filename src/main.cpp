...
int main()
{
    ...
    BT::PortsList payment_check_ports = {
        ...
        BT::OutputPort<bool>("success", "Indicates if payment was successful"),
    };
    factory.registerSimpleCondition("PaymentSuccessful", PaymentSuccessfulCheck, payment_check_ports);

    /* OUTPUT PRODUCT */
    BT::PortsList output_product_ports = {
        BT::InputPort<bool>("success", "Indicates if payment was successful"),
    };
    factory.registerSimpleAction("OutputProduct", [&](BT::TreeNode &self)
                                 { return vm_system.OutputProduct(self); }, output_product_ports);
